...
export default function OrderConfirmation() {
  ...
  useEffect(() => {
    if (state?.orderId && !cleared.current) { cleared.current = true }
  }, [state])
  ...
  return (
    <div className="max-w-[760px] mx-auto px-5 sm:px-8 py-12 sm:py-20">
      ...
      <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <p className="font-mono text-[10px] tracking-[.3em] uppercase text-muted font-bold mb-3">/ Order Placed Successfully</p>
        <h1 className="font-disp text-ink uppercase leading-[.9] mb-4"
          style={{ fontSize: 'clamp(2.5rem,6vw,5rem)' }}>THANK YOU!</h1>
        <p className="font-mono text-[11px] text-muted leading-[1.8] max-w-[400px]">
          Your order has been received. We will contact you on <span className="text-ink font-bold">{form.phone}</span> to confirm and arrange delivery.
        </p>
      </motion.div>

      {/* Order ID */}
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        className="bg-bg2 border border-border p-5 mb-6 flex items-center justify-between flex-wrap gap-3">
        <div>
          <p className="font-mono text-[8px] tracking-[.25em] uppercase text-muted mb-1">/ Order ID</p>
          <p className="font-disp text-ink text-[1.6rem] tracking-[.05em]">{orderId}</p>
        </div>
        <div className="text-right">
          <p className="font-mono text-[8px] tracking-[.25em] uppercase text-muted mb-1">/ Total</p>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        ...
        <div className="p-4 border-b border-border">
          <p className="font-mono text-[8px] tracking-[.25em] uppercase text-muted font-bold">/ Items Ordered</p>
        </div>
        {items.map(item => (
          <div key={item.key} className="flex items-center gap-4 p-4 border-b border-border last:border-0">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-mono text-[11px] text-ink font-bold">{item.product.name}</p>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
        ...
        <div className="border border-border p-5">
          <p className="font-mono text-[8px] tracking-[.25em] uppercase text-muted font-bold mb-3">/ Delivery</p>
        ...
        <div className="border border-border p-5">
          ...
          <p className="font-mono text-[11px] text-ink font-bold">{payLabel}</p>
          {form.screenshot && (
            <img src={form.screenshot} alt="Payment proof" className="h-16 mt-2 object-contain border border-border" />
          )}
          {isCOD && (
            <p className="font-mono text-[10px] text-muted mt-1 leading-[1.7]">Pay <span className="text-ink font-bold">{fmt(total)}</span> when your order arrives.</p>
          )}
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
        className="bg-bg2 border border-border p-6 mb-10">
        <p className="font-mono text-[8px] tracking-[.25em] uppercase text-muted font-bold mb-5">/ What Happens Next</p>
        <div className="space-y-4">
          {[
            ...
            <div key={s.n} className="flex items-start gap-4">
              <span className="font-disp text-ink/20 text-[1.4rem] leading-none flex-shrink-0 w-8">{s.n}</span>
              <div>
                <p className="font-mono text-[10px] text-ink font-bold mb-0.5">{s.t}</p>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
        ...
        <Link to="/shop"
          className="flex-1 sm:flex-none inline-flex items-center justify-center gap-3 bg-ink text-white font-mono text-[9px] font-bold tracking-[.2em] uppercase px-8 py-3.5 hover:bg-ink/80 transition-colors rounded-full">
          CONTINUE SHOPPING →
        </Link>
        <Link to="/"
          className="flex-1 sm:flex-none inline-flex items-center justify-center font-mono text-[9px] tracking-[.2em] uppercase text-muted border border-border px-8 py-3.5 hover:text-ink hover:border-ink transition-all rounded-full">
          GO HOME
        </Link>
      </motion.div>
