...
export default function ProductDetail() {
  ...
  if (!product) return (
    <div className="min-h-screen flex flex-col items-center justify-center">
      <p className="font-disp text-[5rem] text-[rgba(236,236,236,.1)]">NOT FOUND</p>
      <button onClick={() => navigate('/shop')} className="font-mono text-[9px] tracking-[.2em] uppercase text-lime underline mt-4">
        BACK TO SHOP
      </button>
    </div>
  )
  ...
  return (
    <>
      ...
      <div className="max-w-[1440px] mx-auto px-5 sm:px-8 lg:px-12 py-4 border-b border-border flex flex-wrap items-center gap-2 font-mono text-[9px] tracking-[.15em] uppercase text-muted">
        ...
        <span>/</span>
        <Link to="/shop" className="hover:text-lime transition-colors">SHOP</Link>
        <span>/</span>
        <span className="text-lime truncate">{product.name}</span>
      </div>
      ...
      {related.length > 0 && (
        <div className="border-t border-border max-w-[1440px] mx-auto px-5 sm:px-8 lg:px-12 py-14 sm:py-20">
          <p className="font-mono text-[10px] tracking-[.2em] text-lime mb-2">/ YOU MAY ALSO LIKE</p>
      ...
      <div className="absolute bottom-4 left-4 flex gap-1.5 z-10">
        {product.images.map((img, i) => (
          <button key={i} onClick={() => setIdx(i)}
            className={`w-12 h-16 overflow-hidden border transition-all ${i === idx ? 'border-lime opacity-100' : 'border-transparent opacity-40 hover:opacity-70'}`}>
...
function Panel({ product }) {
  ...
  return (
    <div className="border-t lg:border-t-0 lg:border-l border-border px-5 sm:px-8 lg:px-12 py-8 lg:py-12 lg:overflow-y-auto lg:max-h-[calc(100vh-84px)] scrollbar-none">
      ...
      {/* Category + Name */}
      <p className="font-mono text-[9px] tracking-[.25em] uppercase text-lime mb-2">/ {product.category}</p>
      <h1 className="font-disp text-ink uppercase tracking-[.02em] leading-[.9] mb-5"
        style={{ fontSize: 'clamp(2rem,4vw,4rem)' }}>{product.name}</h1>

      {/* Price */}
      <div className="flex items-baseline gap-3 mb-8">
        <span className="font-disp text-[2.2rem] text-lime">{fmt(product.price)}</span>
      ...
      <div className="mb-6">
        <p className="font-mono text-[8px] tracking-[.22em] uppercase text-muted mb-3">
          COLOUR — <span className="text-lime">{color.name}</span>
        </p>
        <div className="flex gap-2">
          {product.colors.map(c => (
            <button key={c.name} onClick={() => setColor(c)} title={c.name}
              className="relative w-6 h-6 transition-transform hover:scale-110"
              style={{ background: c.value, border: '1px solid rgba(236,236,236,.1)' }}>
              {color.name === c.name && <span className="absolute inset-[-4px] border border-lime" />}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="mb-6">
        <p className="font-mono text-[8px] tracking-[.22em] uppercase text-muted mb-3">
          SIZE{size && <span className="text-lime"> — {size}</span>}
        </p>
        <div className="flex flex-wrap gap-2">
          {product.sizes.map(s => (
            <button key={s} onClick={() => { setSize(s); setSizeWarn(false) }}
              className={`font-mono text-[10px] px-4 py-2 border transition-all
                ${size === s ? 'bg-lime text-black border-lime' : 'border-border text-muted hover:border-lime hover:text-lime'}`}>
              {s}
            </button>
          ))}
        </div>
      ...
      <div className="flex items-center gap-4 mb-7">
        ...
        <div className="flex items-center border border-border">
          <button onClick={() => setQty(q => Math.max(1, q - 1))} className="w-9 h-9 flex items-center justify-center text-muted hover:text-lime transition-colors">−</button>
          <span className="w-9 text-center font-mono text-[12px]">{qty}</span>
          <button onClick={() => setQty(q => q + 1)} className="w-9 h-9 flex items-center justify-center text-muted hover:text-lime transition-colors">+</button>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-5">
        <motion.button whileTap={{ scale: 0.98 }} onClick={handleAdd}
          className={`flex-1 py-3.5 font-mono text-[9px] font-bold tracking-[.2em] uppercase transition-colors
            ${added ? 'bg-green-500 text-white' : 'bg-lime text-black hover:bg-[#d4ff5c]'}`}>
